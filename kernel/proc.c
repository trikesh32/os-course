...
reparent(struct proc *p)
{
  ...
  for(struct list2 *pl = list2_begin(&procs); 
      pl != &procs; 
      list2_iterate(&procs, &pl)) {
    release(&procs_lock);
