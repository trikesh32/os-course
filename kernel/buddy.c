...
void *bd_malloc(uint64 nbytes) {
  ...
  bit_toggle(bd_sizes[k].alloc, p_idx / 2);
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    int old_xor = bit_isset(bd_sizes[k].alloc, bi / 2);
...
int bd_initfree_pair(int k, int bi, int is_bi_in_free_reg) {
  int buddy = (bi % 2 == 0) ? bi + 1 : bi - 1;
  int free = 0;
  if (bit_isset(bd_sizes[k].alloc, bi / 2)) {
    // one of the pair is free
    free = BLK_SIZE(k);
    int free_block = is_bi_in_free_reg ? bi : buddy;
    if (free_block == buddy)
